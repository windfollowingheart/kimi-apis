...
import { cryptoJSCode } from "./js/crypto/cryptoJs";
/**
 * 将字节数转换为合适的单位
 * @param bytes 字节数
 * @param precision 精度
...
function parseFileNameAndExtension(path: string): {
    ...
    const fileName = fileNameWithExtension.substring(0, dotIndex) + '.' + fileNameWithExtension.substring(dotIndex + 1);
...
async function calcUint8ArrayHash({uint8Array, cryptoJsUrl}: 
    {
    ...
    if(cryptoJsUrl){
        ...
        self.onmessage = function (e) {
            function uint8ArrayToHexString(uint8Array) {
                const _hexString = Array.from(uint8Array).map((b) => b.toString(16).padStart(2, '0')).join('');
                return _hexString
            }
            try {
        ...
        const _cryptoJSCode = cryptoJSCode
        workerScript = `
        self.onmessage = function (e) {
            const scriptFn = new Function('${_cryptoJSCode}');
            function uint8ArrayToHexString(uint8Array) {
                const _hexString = Array.from(uint8Array).map((b) => b.toString(16).padStart(2, '0')).join('');
                return _hexString
            }
